...
const app = express()
const port = 8000 //у меня на 5000 порту не работал
...
app.get('/tasks', (req, res) => {
    ...
    res.status(200).send({ items, total: items.length, query: req.query })
})
...
app.post('/login', (req, res) => {
    ...
    if (!username || !password) {
        res.status(400).send('Данные должны быть в формате JSON! username и password – обязательные поля')
        return
    }
